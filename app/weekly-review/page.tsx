  ...
  async function load() {
    ...
    for(const h of habitsData) {
      ...
      for(let i=0;i<7;i++){
        ...
        const logsSnap = await getDocs(query(collection(db, 'habit_logs'), where('habit_id','==',h.id), where('logged_date','==',ds), limit(1)))
        if(!logsSnap.empty) done++
        totalPossible++
      }
      totalDone+=done
      ...
      while(true){
        ...
        const logsSnap = await getDocs(query(collection(db, 'habit_logs'), where('habit_id','==',h.id), where('logged_date','==',ds), limit(1)))
        if(!logsSnap.empty){streak++;d2.setDate(d2.getDate()-1)}else break
      }
    ...
    const sportSnap = await getDocs(query(collection(db, 'sport_logs'), where('user_id','==',uid), where('logged_date','>=',weekStart), where('logged_date','<=',weekEnd)))
    if(sportSnap.size>0) {
      const sportDocs = sportSnap.docs.map(d => d.data())
      const avgPerf = Math.round(sportDocs.reduce((a:number,b:any)=>a+b.performance,0)/sportDocs.length)
      const allMissing = sportDocs.flatMap((l:any)=>l.what_was_missing?l.what_was_missing.split(', '):[])
    ...
    const gymSnap = await getDocs(query(collection(db, 'workout_logs'), where('user_id','==',uid), where('logged_date','>=',weekStart), where('logged_date','<=',weekEnd)))
    const prSnap = await getDocs(query(collection(db, 'personal_records'), where('user_id','==',uid), orderBy('created_at','desc'), limit(5)))
    if(gymSnap.size>0) {
      const ins2 = [`${gymSnap.size} gym session${gymSnap.size>1?'s':''} this week`]
      if(!prSnap.empty) {
        const latestPr = prSnap.docs[0].data()
        ins2.push(`Latest PR: ${latestPr.exercise} — ${latestPr.weight_kg}kg × ${latestPr.reps} reps`)
      }
      setGymInsights(ins2)
    }
      ...
      <div className="min-h-screen pb-10" style={{background:'linear-gradient(160deg,#07071a 0%,#0d0920 50%,#070d18 100%)'}}>
        <div className="max-w-lg mx-auto px-4">
          <div className="flex items-center gap-3 pt-8 pb-5">
            <button onClick={()=>router.push('/dashboard')} className="w-9 h-9 rounded-xl flex items-center justify-center text-zinc-400" style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)'}}>
            ...
            <div className="flex flex-col gap-3">
              {habitRates.sort((a,b)=>b.rate-a.rate).map(r=>(
                <div key={r.habit.id}>
                  ...
                  <div className="h-1.5 rounded-full overflow-hidden" style={{background:'rgba(255,255,255,0.06)'}}>
                    <div className="h-full rounded-full" style={{width:`${r.rate}%`,background:r.rate>=70?'#10b981':r.rate>=40?'#f59e0b':'#ef4444'}}/>
                  </div>
                </div>
              ))}
            </div>
