...
export default function RoutineAnalyticsPage() {
  ...
  async function load() {
    ...
    const completionSnap = await getDocs(
      query(collection(db, 'routine_completions'), where('user_id', '==', uid), where('completed_date', '>=', cutoffDateStr))
    )
    const completionData: Completion[] = completionSnap.docs.map(d => ({ id: d.id, ...d.data() } as Completion))
    ...
    for (const routine of routineData) {
      const taskSnap = await getDocs(
        query(collection(db, 'routine_tasks'), where('routine_id', '==', routine.id))
      )
