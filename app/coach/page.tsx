...
export default function AiCoachPage() {
  ...
  async function load() {
    ...
    const thisWeekSnap = await getDocs(
      query(collection(db, 'workout_logs'), where('user_id', '==', uid), where('logged_date', '>=', weekAgoStr))
    )
    const weekVolume = thisWeekSnap.docs.reduce((total: number, d: any) => {
      const w = d.data()
    ...
    const prevWeekSnap = await getDocs(
      query(collection(db, 'workout_logs'), where('user_id', '==', uid), where('logged_date', '>=', twoWeeksAgoStr), where('logged_date', '<', weekAgoStr))
    )
    const prevVolume = prevWeekSnap.docs.reduce((total: number, d: any) => {
      const w = d.data()
      return total + (w.exercises || []).reduce((sum: number, e: any) => {
        return sum + (e.sets || []).reduce((s: number, set: any) => s + ((+set.weight || 0) * (+set.reps || 0)), 0)
      }, 0)
    ...
    const habitsSnap = await getDocs(query(collection(db, 'habits'), where('user_id', '==', uid)))
    const logsSnap = await getDocs(
      query(collection(db, 'habit_logs'), where('user_id', '==', uid), where('logged_date', '>=', weekAgoStr))
    )
    const completionRate = habitsSnap.size > 0 
      ? Math.round((logsSnap.size / (habitsSnap.size * 7)) * 100)
      : 0

    // Get readiness
    const readinessSnap = await getDocs(
      query(collection(db, 'daily_readiness'), where('user_id', '==', uid), where('logged_date', '>=', weekAgoStr))
    )
    const readinessAvg = readinessSnap.size > 0
      ? Math.round(readinessSnap.docs.reduce((a: number, d: any) => a + (d.data().readiness_score || 0), 0) / readinessSnap.size)
      : 0
    ...
    for (let i = 0; i < 365; i++) {
      ...
      const dayLogsSnap = await getDocs(
        query(collection(db, 'habit_logs'), where('user_id', '==', uid), where('logged_date', '==', dateStr))
      )
      if (dayLogsSnap.size > 0) { currentStreak++ } else break
    }

    const newStats = {
      weekVolume, prevVolume, completionRate, currentStreak, readinessAvg,
      gymSessions: thisWeekSnap.size,
    }

    setStats(newStats)
    await generateAdvice(newStats, uid)
    setLoading(false)
  }
  ...
  async function generateAdvice(stats: any, userId: string) {
    ...
    const prompt = `You are a personal fitness coach. Give 4-5 specific, actionable pieces of advice based on this data:
- This week's gym volume: ${stats.weekVolume}kg
- Previous week's volume: ${stats.prevVolume}kg
- Habit completion: ${stats.completionRate}%
- Current streak: ${stats.currentStreak} days
- Average readiness: ${stats.readinessAvg}/100
- Gym sessions: ${stats.gymSessions}

For EACH piece of advice respond with ONLY this JSON format (one per line):
{"type":"insight|warning|encouragement|suggestion","text":"specific advice","emoji":"one emoji"}

Examples: {"type":"encouragement","text":"Your volume increased 12% this week! Keep pushing.","emoji":"💪"}`
    ...
    try {
      ...
      content.split('\n').filter((l: string) => l.trim().startsWith('{')).forEach((line: string) => {
        try { adviceList.push(JSON.parse(line)) } catch (e) {}
      })

      setAdvice(adviceList.length > 0 ? adviceList : [
        { type: 'encouragement', text: `You're at ${stats.completionRate}% completion this week!`, emoji: '💪' },
        { type: 'insight', text: `Readiness is ${stats.readinessAvg}/100 — focus on sleep.`, emoji: '😴' },
      ])
    } catch (error) {
      setAdvice([{ type: 'encouragement', text: 'Keep up the consistency!', emoji: '🔥' }])
    }
    setAnalyzing(false)
  }
  ...
  return (
    <PageContainer title="🤖 AI Coach" subtitle="Personalized guidance">
      ...
      <button onClick={() => load()} disabled={analyzing} className="w-full py-3 rounded-2xl text-sm font-bold text-white transition-all disabled:opacity-50" style={{background:'linear-gradient(135deg,#8b5cf6,#7C3AED)',boxShadow:'0 6px 24px rgba(139,92,246,0.3)'}}>
