...
export default function SettingsPage() {
  ...
  const sections = [
    { title: '⏰ Routines', items: [
        ...
        { icon: '🔥', label: 'Heatmap', path: '/heatmap', desc: 'Heatmap of your habits throughout the year' },
    ]},
        ...
        <div>
          ...
          {showLogout && (
            <div className="mt-3 p-4 rounded-2xl" style={{background:'rgba(239,68,68,0.1)',border:'1px solid rgba(239,68,68,0.2)'}}>
              <p className="text-white text-sm mb-3">Are you sure? You will need to log back in.</p>
              <div className="flex gap-2">
                <button onClick={() => setShowLogout(false)} className="flex-1 py-2 rounded-lg text-sm"
                  style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',color:'rgba(255,255,255,0.5)'}}>Cancel</button>
                <button onClick={handleLogout} className="flex-1 py-2 rounded-lg text-sm font-bold"
                  style={{background:'rgba(239,68,68,0.3)',border:'1px solid rgba(239,68,68,0.5)',color:'#ef4444'}}>Sign Out</button>
              </div>
            </div>
          )}
        </div>
